...
The attention mask is bidirectional within each n_q-token block (one timestep)
and causal across blocks (a timestep cannot see future blocks).
"""
...
def pool_primary_view(
    ...
    spatially avg-pool them to a fixed `out_side x out_side` grid (64 by default).

    Used by `memory_type="vision_feature"`, where the memory module consumes the
    primary view's vision features. backbone_features is left unmodified.
    """
...
class MemoryTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Token i may attend to token j when positions[i] >= positions[j]:
        # bidirectional within a block (same position), causal across blocks.

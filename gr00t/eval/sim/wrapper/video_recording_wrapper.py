...
class VideoRecordingWrapper(gym.Wrapper):
    def __init__(
        ...
        **kwargs,
        ...
        self.episode_id = kwargs.pop("start_episode_id", -1)
        self.name_prefix = kwargs.pop("name_prefix", "episode")
        self.base_seed = kwargs.pop("base_seed", None)
        self.seed_stride = int(kwargs.pop("seed_stride", 100000))
    ...
    def reset(self, **kwargs):
        if kwargs.get("seed", None) is None and self.base_seed is not None:
            episode_seed = int(self.base_seed) * self.seed_stride + int(self.episode_id + 1)
        ...
        if self.video_dir is not None and self.file_path is not None and self.file_path.exists():
            ...
            new_filestem = f"{original_filestem}-" + ("success" if self.is_success else "failure")
        ...
        if self.video_dir is not None:
            self.episode_id += 1
            self.file_path = self.video_dir / f"{self.name_prefix}-episode_{self.episode_id}.mp4"
        return result
